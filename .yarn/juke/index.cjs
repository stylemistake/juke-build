...
var require_glob = __commonJS({
  "pnp:glob-npm-7.1.7-5698ad9c48-b61f48973b.zip/node_modules/glob/glob.js"(exports, module2) {
    ...
    Glob2.prototype._stat = function(f, cb) {
      ...
      function lstatcb_(er, lstat) {
        if (lstat && lstat.isSymbolicLink()) {
          return fs3.stat(abs, function(er2, stat3) {
            if (er2)
...
var File = class {
  ...
  touch() {
    const time = new Date();
    try {
      import_fs.default.utimesSync(this.path, time, time);
    } catch (err) {
      import_fs.default.closeSync(import_fs.default.openSync(this.path, "w"));
    }
  }
};
var Glob = class {
...
var stat = (path) => {
  try {
    return import_fs.default.statSync(path);
  } catch {
    return null;
  }
};
...
var createParameter = (config) => new Parameter(config.name, config.type, config.alias);
var Parameter = class {
...
var Worker = class {
  ...
  async *process() {
    ...
    const fileMapper = async (fileIo) => {
      ...
      return paths.flatMap((path) => path.includes("*") ? new Glob(path).toFiles() : new File(path));
    };
...
var resolveGlob = (globPath) => {
  const unsafePaths = glob2.sync(globPath, {
    strict: false,
    silent: true
  });
  const safePaths = [];
  for (let path of unsafePaths) {
    try {
      import_fs4.default.statSync(path);
      safePaths.push(path);
    } catch {
    }
  }
